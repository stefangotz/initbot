...
class SqlState(State):
    @staticmethod
    def create_with_sqlite(
        sqlite_db_file: str = str(
            Path.home() / ".local" / "var" / "initbot" / "app.sqlite"
        ),
    ) -> "SqlState":
        path = Path(sqlite_db_file)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.touch(exist_ok=True)
        return SqlState.create_with_existing_sqlite_or_fail(sqlite_db_file)

    @staticmethod
    def create_with_existing_sqlite_or_fail(
        sqlite_db_file: str,
    ) -> "SqlState":
        path = Path(sqlite_db_file)
        if not path.exists():
            raise ValueError(f"The database file {path} does not exist")
        try:
            return SqlState(SqliteDatabase(path))
        except Exception as e:
            raise ValueError(
                f"Failed to create SqlState with existing sqlite database: {e}"
            ) from e
